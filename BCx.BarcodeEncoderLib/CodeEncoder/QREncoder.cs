...
namespace BCx
{
   ...
   public class QRCodeOptions : CodeOptions {
      public int                       m_iVersion=-1;
      public QREncoder.ECCLevel        m_eEccLevel=QREncoder.ECCLevel.M;
      public QREncoder.EciMode         m_eEciMode=QREncoder.EciMode.Default;   
   }

    public class QREncoder : CodeEncoder , IDisposable
    {
        ...
        public QREncoder()
        {
         ...
         {
            QRCodeOptions  xQROptions=xOptions as QRCodeOptions;
            //
            if( xQROptions==null ) xQROptions=new QRCodeOptions();
            // todo: detect if Payload is SwissQR
            //
            return CreateQrCode( xPayload.ToString() , 
               xQROptions.m_eEccLevel , false , false , 
               xQROptions.m_eEciMode , 
               xQROptions.m_iVersion ); 
         }

        public CodeData CreateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
        {
        ...
        {
            public static void AddQuietZone(ref CodeData qrCode)
            {
            ...
            public static void PlaceVersion(ref CodeData qrCode, string versionStr)
            {
            ...
            public static void PlaceFormat(ref CodeData qrCode, string formatStr)
            {
                var size = qrCode.ModuleMatrix.Count;
            ...
            public static int MaskCode(ref CodeData qrCode, int version, ref List<Rectangle> blockedModules, ECCLevel eccLevel)
            {
                ...
                foreach (var pattern in methods)
                {
                    ...
                    {
                        var qrTemp = new CodeData(version);
            ...
            public static void PlaceDataWords(ref CodeData qrCode, string data, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
                var up = true;
            ...
            public static void PlaceDarkModule(ref CodeData qrCode, int version, ref List<Rectangle> blockedModules)
            {
                qrCode.ModuleMatrix[4 * version + 9][8] = true;
                blockedModules.Add(new Rectangle(8, 4 * version + 9, 1, 1));
            }

            public static void PlaceFinderPatterns(ref CodeData qrCode, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
            ...
            public static void PlaceAlignmentPatterns(ref CodeData qrCode, List<Point> alignmentPatternLocations, ref List<Rectangle> blockedModules)
            {
            ...
            public static void PlaceTimingPatterns(ref CodeData qrCode, ref List<Rectangle> blockedModules)
            {
                var size = qrCode.ModuleMatrix.Count;
            ...
            private static class MaskPattern
            {
                ...
                public static int Score(ref CodeData qrCode)
                {
                    int score1 = 0,
                        score2 = 0,
                        score3 = 0,
