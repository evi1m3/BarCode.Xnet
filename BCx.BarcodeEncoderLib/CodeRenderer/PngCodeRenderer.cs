...
namespace BCx
{
   public class PngRenderOptions : CodeRenderOptions {

      public string                       m_sFilename;

      public                              PngRenderOptions(int iWidth=256, int iHeight=256)
      {
         m_iWidth=iWidth;
         m_iHeight=iHeight;
      }
    ...
    public class PngRenderer : CodeRenderer, IDisposable
    {
        CodeData                          m_xCodeData;
        ...
        public void                       Dispose()
        {
         ...
         public override void             Render(CodeData xData)
         {
            ...
            //
            if( xOpt==null ) xOpt=new PngRenderOptions();
            //
            m_xCodeData=xData;
            //
        ...
        public byte[] GetGraphic(int pixelsPerModule)
        {
            ...
            {
                var sizex = this.m_xCodeData.ModuleMatrix[0].Count * pixelsPerModule;
                var sizey = this.m_xCodeData.ModuleMatrix.Count * pixelsPerModule;
                png.WriteHeader(sizex, sizey, 1, PngBuilder.ColorType.Greyscale);
        ...
        {
            var moduleMatrix = this.m_xCodeData.ModuleMatrix;
            var matrixSizeX = moduleMatrix[0].Count;
            var matrixSizeY = moduleMatrix.Count;
            var bytesPerScanline = (matrixSizeX * pixelsPerModule + 7) / 8 + 1; // A monochrome scanline is one byte for filter type then one bit per pixel.
            var scanlines = new byte[bytesPerScanline * matrixSizeX * pixelsPerModule];

            for (var y = 0; y < matrixSizeY; y++)
            {
